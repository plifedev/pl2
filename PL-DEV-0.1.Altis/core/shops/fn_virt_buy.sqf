...
_amount = _diff;
if((_price * _amount) > uj748rtaz1) exitWith {hint localize "STR_NOTF_NotEnoughMoney"};
...
{
	if((_price * _amount) > uj748rtaz1) exitWith {hint localize "STR_NOTF_NotEnoughMoney"; [false,_type,_amount] call life_fnc_handleInv;};
	hint format[localize "STR_Shop_Virt_BoughtItem",_amount,(localize _name),[(_price * _amount)] call life_fnc_numberText];
	SUB(uj748rtaz1,(_price * _amount));
	playSound "caching";
