[] spawn {
	...
	while {true} do
	{
		...
		(_uiDisp displayCtrl 1000)ctrlSetText format["Militaires: %1 | Pompiers: %3 | Civils: %2 | Dépanneurs: %5 | Position: %4",  west countSide playableUnits, civilian countSide playableUnits, independent countSide playableUnits, mapGridPosition player,{_x getVariable ["job", 0] == 1} count playableUnits, _counter];
	};
};
